...
public class Receiver {
	...
	private void receiveFile(Packet packet) throws Exception {
		...
		DatagramPacket sendPacket = new DatagramPacket(packetBytes, packet.getBytes().length , IPAddress, Settings.PORT);
		...
		while (!haveEOT) {
			try {
				...
				// ...
				Packet receiveData2 = new Packet(receiveData);
				incomingDAT = receiveData2;
				

				// If the packet type of incoming data is an error (ERR) log it and exit this method
				 if (incomingDAT.getType() == PacketType.ERR) {
					logger.info("Error: ");
					return;
				}
		...
		// Print out the file
		System.out.println("HERE: " + holdFile);
	}
