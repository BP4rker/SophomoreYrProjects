		...
		try {
			...
			while (indication) {
				...
				try {
					...
					// of 0.
					// hint: byte[] buf ...
					byte[] buffer = udpSocket.getPacket(0);
					
					// convert the buf into a new Packet and store it in the receivedPacket
					// hint: receivedPacket = ...
					Packet pacBuffer = new Packet(buffer);
					receivedPacket = pacBuffer;
				...
				try {
					...
					// ...
					byte[] receiveArray = receivedPacket.getData();
					String check = new String(receiveArray);
					System.out.println("Beta DAT");
					System.out.println(check);
					sender.sendFile(check);
